  ...
  def generate(self, shapes):
    ...
    for shape in shapes:
      if 'shape' in shape:
        ...
        self.label(f, shape)
    f.write("</package>\n");
    return BeautifulSoup(f.getvalue(), "xml")
  ...
  def export(self, fn, shapes):
    ...
    for package in packages:
      if package['name'] == name:
         found = package
         break
    # warning, this creates incorrect XML... WIP
    gen = self.generate(shapes)
    if found != None:
      found.replace_with(gen)
    else:
      soup.eagle.drawing.packages.append(gen)
    print soup.prettify()
    # BUSY
