    ...
    def export_footprint(self, interim):
        ...
        def pad(shape, smd=False):
            ...
            if shape2 == 'disc':
                ...
                if ro == 100:
                    ...
                    if ro == 0:
                        ...
                        # to not overrun the pad, however it is tricky to make
                        # sure the 4 lines don't create any holes.
                        small = dx
                        if dy < dx:
                            small = dy
                        radius = small / 2 * ro / 100.0
                        l.append([S('size'), dx - radius * 2, dy - radius * 2])
                        gen_lines(shapes, fget(shape, 'x'), -fget(shape,
                                                                  'y'), iget(shape, 'rot'), radius, dx, dy)
            else:
                raise Exception(
                    "%s shaped pad not supported in kicad" % (shape2))
            l.append([S('at'), fget(shape, 'x'), -
                      fget(shape, 'y'), iget(shape, 'rot')])
            if smd:
                if shape['type'] != 'smd':
                    l.append([S('layers'), S('F.Paste')])
                else:
                    if want_paste:
                        l.append([S('layers'), S('F.Cu'),
                                  S('F.Paste'), S('F.Mask')])
                    else:
            ...
            if not smd:
                ...
                if 'drill_off_dx' in shape or 'drill_off_dy' in shape:
                    l2.append(
                        [S('offset'), fget(shape, 'drill_off_dx'), fget(shape, 'drill_off_dy')])
                l.append(l2)
        ...
        def vertex(shape, layer):
            if not 'curve' in shape or shape['curve'] == 0.0:
                ...
                ((x0, y0), r, a1, a2) = calc_center_r_a1_a2(
                    (x1, y1), (x2, y2), angle)
        ...
        def circle(shape, layer):
            ...
            if not 'a1' in shape and not 'a2' in shape:
                ...
                l.append([S('end'), fc(x + (r / math.sqrt(2))),
                          fc(y + (r / math.sqrt(2)))])
                l.append([S('layer'), S(layer)])
                l.append([S('width'), fget(shape, 'w')])
            else:
        ...
        def disc(shape, layer):
            ...
            r = fget(shape, 'r')
            rad = r / 2
            l.append([S('end'), x + (rad / math.sqrt(2)),
                      y + (rad / math.sqrt(2))])
            l.append([S('layer'), S(layer)])
        ...
        def label(shape, layer):
            ...
            if (('rot' in shape) and (fget(shape, 'rot') != 0.0)):
                l.append([S('at'), fget(shape, 'x'),
                          fc(-fget(shape, 'y')), fget(shape, 'rot')])
            else:
        ...
        def silk(shape):
            if not 'shape' in shape:
                return None
            layer = type_to_layer_name(shape['type'])
            s = shape['shape']
            if s == 'line':
                return vertex(shape, layer)
            if s == 'vertex':
                return vertex(shape, layer)
            elif s == 'circle':
                return circle(shape, layer)
            elif s == 'disc':
                return disc(shape, layer)
            elif s == 'label':
                return label(shape, layer)
            elif s == 'rect':
                return rect(shape, layer)
            elif s == 'polygon':
                return polygon(shape, layer)
    ...
    def _import_footprint(self, s):
        ...
        def get_single_element_sub(x, name, default=None):
            ...
            if (sub != None):
                if (len(sub) != 1):
                    raise Exception(
                        "Unexpected multi-element '%s' sub: %s" % (name, str(sub)))
                return sub[0]
            return default
